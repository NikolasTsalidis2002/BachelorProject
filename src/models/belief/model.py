...
from src.models.belief.agentLLM import BeliefLLMAgent

class BeliefLLMModel(GridModel):
    def __init__(self, config, id="", dynamic=False,  param_name =None):

        title="Belief Propagation LLM"
        ...
        for i in range(len(self.personas)):
            for j in range(num_agents_by_type[i]):
                ...
                self.agents[self.positions[count]] = BeliefLLMAgent(
                    self.config, state=state, position=self.positions[count], persona=persona, extra_prompt=extra_prompt, message=message, client=self.client
                )
                count += 1
