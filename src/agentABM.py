...
class GridABMAgent(ABMAgent):
    ...
    def get_neighbors(self, agents, k=1):
        #offsets is a combination of all the coordinates in range from -1 to 1 excluding 0,0 (itself, where the agent is)
        # meaning, it is going to be lookingg at all 8 closest coordinates that it could possibly move to
        offsets = list(product(range(-k, k+1), repeat=len(self.position)))
        offsets.remove((0,) * len(self.position))
        neighbors = []
        # go thoruggh every possible coordinate the agent could move to
        for offset in offsets: 
            neighbor_pos = tuple(self.position[i] + offset[i] for i in range(len(self.position)))
            # if the said coordinate has an agent, then we will consider this as a neighbor
            if neighbor_pos in agents:
                neighbors.append(agents[neighbor_pos])
        return neighbors # return a list of all the coordinates where there are neighbours (no matter their persona)
